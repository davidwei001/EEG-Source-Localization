function [x_lambda,T] = yl_tikhonov(U,s,V,b,lambda,x_0)
%TIKHONOV Tikhonov regularization.
%
% [x_lambda,T] = tikhonov(U,s,V,b,lambda,x_0)
% [x_lambda,T] = tikhonov(U,sm,X,b,lambda,x_0) ,  sm = [sigma,mu]
%
...
if (ps==1)
    ...
    s = (s.^2+lambda^2)./s;    % equivalent singular value (after apply filter factor)
    s_inv = 1./s;
    s_inv = diag(s_inv);
    T = V * s_inv * U';
    x_lambda = T*b;
...
%     
%     if (nargin==5)
%         a=zeta./(s.^2 + lambda^2);size(a)
%         x_lambda = V(:,1:p)*(zeta./(s.^2 + lambda^2));
%     else
%         x_lambda = V(:,1:p)*...
%             ((zeta + lambda^2*omega)./(s.^2 + lambda^2));
%     end

else

  % The underdetermined general-form case.  (I haven't modified this case)
  gamma2 = (s(:,1)./s(:,2)).^2;
